...
- 任一应有对白未成功生成时，独立 TTS 仍会按现有策略重试；已下载的模型成片优先保留，分辨率、时长、帧、音轨、采样率或完整解码结果只写入任务记录，不再阻断镜头完成。
- 最终剪辑使用 `FINAL_VIDEO_PRESET=slow`、`FINAL_VIDEO_CRF=14` 的高质量编码；最终 QC 只作为交付报告中的检查信息，文件生成后继续进入交付状态。
