...
import {
    ...
    analyzeAndBackfillScriptCharacters,
    analyzeProjectBrief,
    analyzeProjectStyle,
    cancelRenderJob,
    comfyPreflight,
...
import { summarizeRenderJobs, summarizeShotLocks } from "@/lib/render-job-stats";
import { isH3PromptBlocked, isKeyframeBusy } from "@/lib/production-busy";
import { buildStoryboardDialoguePatch, getStoryboardDialogueText } from "@/lib/storyboard-dialogue";
...
import PromptReferenceEditor, { PromptMaterialThumbnail, type PromptReferenceMaterial } from "@/components/PromptReferenceEditor";
import type { Asset, AssetRole, CharacterProfile, Delivery, H3MaterialDiagnostics, H3PromptSkill, KeyframeMaterialDiagnostics, Project, ProjectAnalysisDraft, ProjectBundle, RenderJob, ScriptDurationAssessment, ScriptRewriteDraft, SeedanceCatalog, SeedanceEstimate, SeedanceMaterialDiagnostics, Shot, ShotSplitPreview, StyleAnalysisDraft } from "@/types";
...
const INSERT_BEFORE_FIRST = "__before_first__";
...
function labelPromptMaterials(items: Array<Pick<Asset, "id" | "name" | "type">>): PromptReferenceMaterial[] {
    const counters: Record<"image" | "video" | "audio", number> = { image: 0, video: 0, audio: 0 };
    return items
        .filter((item): item is Pick<Asset, "id" | "name" | "type"> & { type: "image" | "video" | "audio" } => item.type === "image" || item.type === "video" || item.type === "audio")
        .map((item) => {
            counters[item.type] += 1;
            const prefix = item.type === "image" ? "图片" : item.type === "video" ? "视频" : "音频";
            return { ...item, label: `${prefix}${counters[item.type]}` };
        });
}
    ...
    return (
        <main className="min-h-screen bg-[#080a0f] text-white">
            <header className="sticky top-0 z-40 border-b border-white/8 bg-[#0d1017]/95 backdrop-blur-xl">
                <div className="mx-auto flex max-w-[1700px] items-center gap-4 px-4 py-3 md:px-8">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-white/35">{project.brief.project_quote == null ? "未填写项目报价" : `项目报价 ¥${project.brief.project_quote.toFixed(2)}`} · {project.brief.target_duration_seconds}s · {project.brief.aspect_ratio}</p>
                    </div>
            ...
            <div className="mx-auto max-w-[1700px] px-4 py-6 md:px-8">
                ...
                {tab === "storyboard" && <StoryboardPanel bundle={bundle} busy={busyFor("storyboard", "storyboard-import", "ai-revise", "reorder", "blank-shot", "shot", "delete")} action={action} refresh={refresh} updateLocal={(shots) => setBundle({ ...bundle, shots })} setProject={(next) => setBundle((current) => current ? { ...current, project: next } : current)} />}
                {tab === "assets" && <AssetsPanel project={project} assets={bundle.assets} shots={bundle.shots} busy={busy} action={action} />}
                {tab === "production" && <ProductionPanel bundle={bundle} busyTasks={busy} busy={busyFor("preflight", "keyframes", "h3-prompts", "seedance-prompts", "export-keyframes", "export-videos", "plan", "render", "h3", "seedance", "preset", "upload-keyframe", "keyframe-prompt", "cancel", "delete")} error={error} notice={notice} action={action} refresh={refresh} notify={(message) => { setError(""); setNotice(message); }} fail={(message) => { setNotice(""); setError(message); }} />}
                {tab === "review" && <ReviewPanel bundle={bundle} busy={busyFor("approve", "changes")} action={action} />}
                {tab === "delivery" && <DeliveryPanel bundle={bundle} busy={busyFor("cover", "subtitle", "finalize")} action={action} />}
            </div>
        </main>
    );
}
...
function BriefPanel({ project, assets, busy, action, setLocal, save }: { project: Project; assets: Asset[]; busy: BusyState; action: Action; setLocal: (project: Project) => void; save: (project: Project) => Promise<void> }) {
    ...
    const changeBrief = (key: keyof Project["brief"], value: string | number | null) => {
    ...
    const runCharacterBackfill = async () => {
        setAnalysisBusy("character-backfill"); setAnalysisError(""); setAnalysisNotice("");
        try {
            await save(project);
            const result = await analyzeAndBackfillScriptCharacters(project.id);
            setLocal(result.project);
            setAnalysisNotice(result.message);
        } catch (caught) {
    ...
    return <div className="grid gap-5 xl:grid-cols-[1.3fr_.7fr]">
        <section className="studio-panel">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="项目报价（元）"><input type="number" min={0} step="0.01" value={project.brief.project_quote ?? ""} onChange={(event) => changeBrief("project_quote", event.target.value === "" ? null : Math.max(0, Number(event.target.value)))} placeholder="选填，例如：6800" /></Field>
                ...
                <Field label="口播节奏"><select value={project.brief.speech_pacing} onChange={(event) => changeBrief("speech_pacing", event.target.value)}><option value="natural">自然口播 · 3.4 字/秒</option><option value="brisk">紧凑讲解 · 4.8 字/秒</option><option value="short_ad">抓眼快口播广告 · 6.2 字/秒</option></select><p className="mt-1 text-[11px] leading-5 text-white/35">保存为系列后，新一集会自动继承该口播档。</p></Field>
                ...
                <Field label="Seedance 系列强制约束" wide><textarea rows={3} value={project.seedance_global_constraints} onChange={(event) => setLocal({ ...project, seedance_global_constraints: event.target.value })} placeholder="例如：所有出镜人物全程佩戴安全帽；林晓的头发全部收进安全帽内，帽外不得露发。" /><p className="mt-1 text-[11px] leading-5 text-white/30">会自动放在每个 Seedance Prompt 的开头和结尾；更新角色资料或刷新时仍会保留。每镜手工补充的约束也会单独保存。</p></Field>
        ...
        <section className="studio-panel">
            <div className="mb-5 flex items-center justify-between gap-3"><div><p className="studio-kicker">CHARACTER BIBLE</p><h2 className="text-xl font-semibold">角色一致性</h2></div><div className="flex flex-wrap justify-end gap-2"><button className="studio-secondary text-xs" title="只分析当前剧本角色；已有角色保持不变，只追加缺失角色" disabled={!!analysisBusy || !project.brief.story.trim()} onClick={() => void runCharacterBackfill()}>{analysisBusy === "character-backfill" ? <Loader2 className="animate-spin" size={14} /> : <WandSparkles size={14} />}AI 分析并补足角色</button><button className="studio-secondary" onClick={addCharacter}><Plus size={15} />角色</button></div></div>
...
function StoryboardPanel({ bundle, busy, action, refresh, updateLocal, setProject }: { bundle: ProjectBundle; busy: string; action: Action; refresh: () => Promise<void>; updateLocal: (shots: Shot[]) => void; setProject: (project: Project) => void }) {
    ...
    const [spokenTextPolicy, setSpokenTextPolicy] = useState<"adaptive" | "verbatim">(bundle.project.brief.spoken_text_policy || "adaptive");
    ...
    const submitGeneration = () => {
        ...
        void action(
            "storyboard",
            () => generateStoryboard(bundle.project.id, countMode === "manual" ? count : undefined, countMode, userSuggestions.trim(), promptTargets, "h3-prompt-writing", spokenTextPolicy),
    ...
    const submitInsert = () => {
        ...
        const insertBeforeFirst = insertAfterShotId === INSERT_BEFORE_FIRST;
        const beforeShot = insertBeforeFirst ? bundle.shots[0] : undefined;
        const afterShot = insertBeforeFirst
            ? undefined
            : insertAfterShotId
                ? bundle.shots.find((shot) => shot.id === insertAfterShotId)
                : bundle.shots.at(-1);
        const afterId = afterShot?.id || null;
        const beforeId = beforeShot?.id || null;
        const insertionLabel = beforeId
            ? `镜头 ${beforeShot?.ordinal || 1} 前`
            : afterId === bundle.shots.at(-1)?.id
                ? "结尾"
                : `镜头 ${afterShot?.ordinal || 0} 后`;
        setInsertOpen(false);
        void action(
            "insert-shot",
            () => insertShotWithAi(
    ...
    return <div>
        ...
        {bundle.shots.length === 0 ? <div className="studio-empty"><LayoutList size={30} className="text-cyan-300" /><strong>还没有分镜</strong><span>{countMode === "ai" ? "让 AI 判断镜头数，或切换为手动数量后生成" : "设置镜头数后点击“AI 生成/重做”"}</span></div> : <div className="space-y-3">
            {bundle.shots.map((shot, index) => {
                const storyboardPromptIds = [shot.keyframe_asset_id, ...(shot.video_reference_asset_ids ?? shot.reference_asset_ids)].filter((value): value is string => !!value);
                const storyboardPromptMaterials = labelPromptMaterials(
                    [...new Set(storyboardPromptIds)]
                        .map((assetId) => bundle.assets.find((asset) => asset.id === assetId))
                        .filter((asset): asset is Asset => !!asset),
                );
                return <article key={shot.id} className="studio-panel !p-0 overflow-hidden">
                ...
                {open === shot.id && <div className="border-t border-white/8 p-4 md:p-5">
                    <div className="grid gap-4 md:grid-cols-3">
                        ...
                        <Field label="镜头连续方式（Seedance / H3）"><select value={shot.continuity_mode} onChange={(event) => patchShot(shot.id, { continuity_mode: event.target.value as Shot["continuity_mode"], continuity_source_shot_id: event.target.value === "continuous" ? (bundle.shots[index - 1]?.id || null) : null })}><option value="independent">独立镜头 · 正常切镜</option><option value="same_scene">同场景切镜 · 不硬接尾帧</option><option value="continuous" disabled={index === 0}>连续长镜头 · 接上一镜尾帧</option></select><span className="mt-1 block text-[11px] leading-5 text-white/35">这个设置同时适用于 Seedance 与 H3；连续模式读取上一镜真实尾帧，普通换机位选“同场景切镜”。</span></Field>
                        ...
                        <Field label="对白/旁白"><textarea rows={3} value={getStoryboardDialogueText(shot)} onChange={(event) => patchShot(shot.id, buildStoryboardDialoguePatch(shot, event.target.value))} /><span className="mt-1 block text-[11px] leading-5 text-white/35">{!shot.dialogue.trim() && shot.voice_events.length > 0 ? `已从 ${shot.voice_events.length} 段结构化声音同步显示；逐行修改会保留说话人与时间。` : "修改整段对白后，系统会在保存/编译计划时重新分析逐句发言者。"}</span></Field>
                        ...
                        <Field label="对白语速调整（%）"><input type="number" min={-50} max={100} step={5} value={shot.dialogue_rate_percent} onChange={(event) => patchShot(shot.id, { dialogue_rate_percent: Math.min(100, Math.max(-50, Number(event.target.value))) })} /><span className="mt-1 block text-[11px] leading-5 text-white/35">正数加快，负数放慢；快口播会保留原文并写入 H3 节奏约束，极端超载会在创建付费任务前拦截。</span></Field>
                        ...
                        <Field label={`MiniMax H3 Prompt · ${shot.h3_prompt_skill_id || "h3-prompt-writing"}`} wide><PromptReferenceEditor value={shot.video_prompt} materials={storyboardPromptMaterials} getAssetUrl={(asset) => projectInlineUrl(bundle.project.id, "asset", asset.id)} onChange={(value) => patchShot(shot.id, { video_prompt: value })} placeholder="输入或粘贴 H3 Prompt…" /><span className="mt-1 block text-[11px] leading-5 text-white/35">可手动微调；在“视频生成”页可选择官方风格 Skill 批量重新生成。</span></Field>
                        <Field label={`Seedance 2.0 全模态 Prompt${shot.seedance_prompt_version ? ` · ${shot.seedance_prompt_version}` : ""}`} wide><PromptReferenceEditor value={shot.seedance_prompt} materials={storyboardPromptMaterials} getAssetUrl={(asset) => projectInlineUrl(bundle.project.id, "asset", asset.id)} onChange={(value) => patchShot(shot.id, { seedance_prompt: value })} placeholder="在“视频生成”页点击编译 Seedance Prompt 后自动生成，也可在这里手动微调。" /><span className="mt-1 block text-[11px] leading-5 text-white/35">使用图片1/视频1/音频1编号，与方舟 content 数组顺序严格一致；不会覆盖 H3 Prompt。</span></Field>
                    </div>
        ...
        {insertOpen && <div className="studio-modal" onMouseDown={() => setInsertOpen(false)}><section className="studio-dialog max-w-2xl" onMouseDown={(event) => event.stopPropagation()}><p className="studio-kicker">AI INSERT ONE SHOT</p><div className="mb-5 flex items-start gap-3"><span className="rounded-xl bg-cyan-300/10 p-3 text-cyan-200"><Plus size={22} /></span><div><h2 className="text-2xl font-semibold">单独新增一个完整分镜</h2><p className="mt-1 text-sm leading-6 text-white/45">只生成并插入这一镜；现有分镜、首帧、H3 Prompt、Seedance Prompt 和视频结果都保留。</p></div></div><div className="grid gap-4"><Field label="插入位置"><select value={insertAfterShotId} onChange={(event) => setInsertAfterShotId(event.target.value)}><option value="">作为新的结尾镜头</option>{bundle.shots.length > 0 && <option value={INSERT_BEFORE_FIRST}>插在镜头 {bundle.shots[0].ordinal} 前（新增开场 / 过渡镜头）</option>}{bundle.shots.slice(0, -1).map((shot) => <option key={shot.id} value={shot.id}>插在镜头 {shot.ordinal} 后</option>)}</select></Field><label><span className="studio-label">新增镜头要求</span><textarea className="studio-input min-h-44 resize-y" maxLength={4000} value={insertSuggestions} onChange={(event) => setInsertSuggestions(event.target.value)} placeholder="例如：在当前第一个镜头前增加一段承接上一集结尾的过渡画面，再自然进入本集开场；或者在结尾增加反转镜头。" autoFocus /></label><div className="rounded-lg border border-cyan-300/10 bg-cyan-300/[.03] p-4"><span className="studio-label">同时生成哪些视频 Prompt</span><div className="mt-2 flex flex-wrap gap-5 text-sm text-white/65"><label className="flex items-center gap-2"><input type="checkbox" checked={promptTargets.includes("h3")} onChange={() => setPromptTargets(toggle(promptTargets, "h3") as ("h3" | "seedance")[])} />MiniMax H3（按需）</label><label className="flex items-center gap-2"><input type="checkbox" checked={promptTargets.includes("seedance")} onChange={() => setPromptTargets(toggle(promptTargets, "seedance") as ("h3" | "seedance")[])} />Seedance 2.x（本地编译）</label></div></div></div><div className="mt-6 flex justify-end gap-2"><button className="studio-secondary" onClick={() => setInsertOpen(false)}>取消</button><button className="studio-primary" disabled={!insertSuggestions.trim() || !!busy} onClick={submitInsert}>{busy === "insert-shot" ? <Loader2 className="animate-spin" size={15} /> : <WandSparkles size={15} />}生成并插入这一镜</button></div></section></div>}
...
function AssetsPanel({ project, assets, shots, busy, action }: { project: Project; assets: Asset[]; shots: Shot[]; busy: BusyState; action: Action }) {
    ...
    return <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
        <section className="studio-panel h-fit">
            ...
            <p className="mt-4 text-xs leading-5 text-white/35">{shots.length === 0 ? "请先生成或导入分镜。场景档案需要从已有分镜中归纳重复空间。" : "第一步识别空间文字规范；第二步可上传本地场景图，或生成并修正 AI 母版；最后在下方一键应用到所有已归类镜头，不需要逐镜勾选。"}</p>
        </section>
...
type VideoPromptInputMode = "" | "frame" | "reference";

const H3_API_RATIOS: Shot["h3_ratio"][] = ["project", "adaptive", "21:9", "16:9", "4:3", "1:1", "3:4", "9:16"];
...
type ShotCardMediaProps = {
    ...
    previewReference?: (asset: Asset) => void;
};

function H3ShotCard({ shot, project, assets, checked, busy, directorVersion, toggleChecked, save, projectId, keyframe, generatingKeyframe, editKeyframe, previewKeyframe, submit, previousShotId, previewReference }: { shot: Shot; project: Project; assets: Asset[]; checked: boolean; busy: boolean; directorVersion: string; toggleChecked: () => void; save: (shot: Shot) => Promise<void>; previousShotId: string | null } & ShotCardMediaProps) {
    const [draft, setDraft] = useState(shot);
    const [referencePreview, setReferencePreview] = useState<Asset | null>(null);
    const [h3Diagnostics, setH3Diagnostics] = useState<H3MaterialDiagnostics | null>(null);
    useEffect(() => {
        let cancelled = false;
        void getH3Materials(projectId, shot.id).then((result) => {
            if (!cancelled) setH3Diagnostics(result);
        }).catch(() => {
            if (!cancelled) setH3Diagnostics(null);
        });
    ...
    const hasFirstFrame = !!(draft.keyframe_asset_id || draft.image_path);
    const hasLastFrame = !!draft.last_frame_asset_id;
    ...
    const effectiveMode = draft.generation_mode === "auto"
        ? (hasLastFrame ? "i2v" : dialogueGuidedR2v ? "r2v" : hasFirstFrame ? "i2v" : draft.reference_asset_ids.length ? "r2v" : "i2v")
        : draft.generation_mode;
    const referenceAssets = draft.reference_asset_ids.map((assetId) => assets.find((asset) => asset.id === assetId)).filter((asset): asset is Asset => !!asset);
    const h3MaterialIds = h3Diagnostics?.materials.map((item) => item.id)
        ?? (draft.video_reference_asset_ids ?? [draft.keyframe_asset_id, ...draft.reference_asset_ids].filter((value): value is string => !!value));
    const h3PromptMaterials = labelPromptMaterials(
        [...new Set(h3MaterialIds)]
            .map((assetId) => assets.find((asset) => asset.id === assetId))
            .filter((asset): asset is Asset => !!asset),
    );
    const imageReferenceCount = referenceAssets.filter((asset) => asset.type === "image").length + (effectiveMode === "r2v" && hasFirstFrame ? 1 : 0);
    const videoReferenceCount = referenceAssets.filter((asset) => asset.type === "video").length;
    const audioReferenceCount = referenceAssets.filter((asset) => asset.type === "audio").length;
    const totalReferenceCount = imageReferenceCount + videoReferenceCount + audioReferenceCount;
    const inputLabel = effectiveMode === "r2v"
        ? "全能参考"
        : hasFirstFrame && hasLastFrame
            ...
            : hasFirstFrame
                ...
                : hasLastFrame
                    ? "尾帧"
                    : "帧模式待绑定";
    const resolutionLabel = draft.h3_resolution === "default" ? "默认清晰度" : draft.h3_resolution;
    const ratioLabel = effectiveMode === "i2v"
        ? "比例由输入帧决定"
        : draft.h3_ratio === "project"
            ? project.brief.aspect_ratio
            : draft.h3_ratio;
    const tuningWarnings = [
        effectiveMode === "i2v" && !hasFirstFrame && !hasLastFrame ? "帧模式至少要绑定首帧或尾帧；首尾都绑定时会按首尾帧过渡生成。" : "",
        effectiveMode === "r2v" && totalReferenceCount === 0 ? "全能参考模式至少需要一项图片、视频或音频素材。" : "",
        effectiveMode === "i2v" && draft.reference_asset_ids.length > 0 ? "帧模式与全能参考互斥；下方已勾选的参考素材本次不会提交。" : "",
        effectiveMode === "r2v" && hasLastFrame ? "全能参考模式不会提交尾帧；如需精确到达尾帧，请改用帧模式。" : "",
        imageReferenceCount > 9 ? `参考图片共 ${imageReferenceCount} 张，超过 MetaSo H3 的 9 张上限。` : "",
        videoReferenceCount > 3 ? `参考视频共 ${videoReferenceCount} 个，超过 MetaSo H3 的 3 个上限。` : "",
        audioReferenceCount > 3 ? `参考音频共 ${audioReferenceCount} 个，超过 MetaSo H3 的 3 个上限。` : "",
        totalReferenceCount > 12 ? `全能参考共 ${totalReferenceCount} 项，超过 MetaSo H3 的 12 项总上限。` : "",
        draft.h3_resolution === "2K" ? "2K 会提高细节和单秒费用，适合确认后的最终版本。" : "",
        draft.h3_context_ir_enabled === true ? "Context IR 已开启，MetaSo 会按次收取额外的素材理解费用。" : "",
    ].filter(Boolean);
    return <article className="rounded-xl border border-white/8 bg-black/15 p-4">
        {h3PromptMaterials.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-cyan-300/10 bg-cyan-300/[.025] px-3 py-2"><span className="mr-1 text-[11px] text-white/40">参考缩略图</span>{h3PromptMaterials.map((material) => { const asset = assets.find((item) => item.id === material.id); return <span key={material.id} className="inline-flex items-center gap-1.5 text-[11px] text-white/60"><PromptMaterialThumbnail material={material} getAssetUrl={(item) => projectInlineUrl(projectId, "asset", item.id)} onClick={asset?.type === "image" && previewReference ? () => previewReference(asset) : undefined} /><span>{material.label}</span></span>; })}</div>}
        <div className="flex flex-wrap items-start gap-3">
            ...
            <span className="studio-status uppercase">METASO · {draft.generation_mode === "auto" ? `AUTO→${inputLabel}` : inputLabel}</span>
            <span className="rounded bg-cyan-300/8 px-2 py-1 font-mono text-[11px] text-cyan-100/70">{resolutionLabel} · {ratioLabel} · {Math.round(draft.duration_seconds)}秒</span>
        </div>
        <div className="mt-4 grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]"><ShotKeyframeSummary shot={shot} keyframe={keyframe} projectId={projectId} generating={generatingKeyframe} edit={editKeyframe} preview={previewKeyframe} /><div><span className="studio-label">MiniMax H3 Prompt</span><PromptReferenceEditor value={draft.video_prompt} materials={h3PromptMaterials} getAssetUrl={(asset) => projectInlineUrl(projectId, "asset", asset.id)} onChange={(value) => patch({ video_prompt: value })} onReferenceClick={(material) => { const asset = assets.find((item) => item.id === material.id); if (!asset) return; if (asset.type === "image") { if (previewReference) previewReference(asset); else setReferencePreview(asset); } else window.open(projectInlineUrl(projectId, "asset", asset.id), "_blank", "noopener,noreferrer"); }} placeholder="输入或粘贴 H3 Prompt…" /><span className="mt-1.5 block text-[11px] text-white/32">首帧与本镜 H3 Prompt 放在同一卡片；提交本镜前可一起核对。</span></div></div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Field label="H3 镜头连续方式" hint="连续长镜头会读取上一镜 H3 已采用版本的真实尾帧，作为本镜首帧；第一镜从这里开始不可选。普通换机位请选择同场景切镜。"><select value={draft.continuity_mode} onChange={(event) => { const mode = event.target.value as Shot["continuity_mode"]; patch({ continuity_mode: mode, continuity_source_shot_id: mode === "continuous" ? previousShotId : null }); }}><option value="independent">独立镜头 · 正常切镜</option><option value="same_scene">同场景切镜 · 不硬接尾帧</option><option value="continuous" disabled={!previousShotId}>连续长镜头 · 接上一镜尾帧</option></select><span className="mt-1 block text-[11px] leading-5 text-white/35">选中后先保存本镜参数；批量提交时按镜头顺序排队，上一镜完成后自动取真实尾帧续接。</span></Field>
            <Field label="MetaSo 输入模式" hint="帧模式可只给首帧、只给尾帧或同时给首尾帧；全能参考可组合图片、视频和音频。两类输入不会混在同一次请求中。"><select value={draft.generation_mode} onChange={(event) => patch({ generation_mode: event.target.value as Shot["generation_mode"] })}><option value="auto">自动判断</option><option value="i2v">帧模式</option><option value="r2v">全能参考</option></select></Field>
            <Field label="接口清晰度" hint="768P 适合批量预览和多数短视频；2K 细节更高，同时按更高单秒价格计费。"><select value={draft.h3_resolution} onChange={(event) => patch({ h3_resolution: event.target.value as Shot["h3_resolution"] })}><option value="default">沿用模型设置</option><option value="768P">768P</option><option value="2K">2K</option></select></Field>
            <Field label="接口画面比例" hint={effectiveMode === "i2v" ? "帧模式最终比例由输入帧决定；这里会保留设置，切到全能参考时生效。" : "跟随项目会使用当前项目画幅；也可以为本镜单独指定。"}><select value={draft.h3_ratio} onChange={(event) => patch({ h3_ratio: event.target.value as Shot["h3_ratio"] })}>{H3_API_RATIOS.map((ratio) => <option key={ratio} value={ratio}>{ratio === "project" ? `跟随项目 · ${project.brief.aspect_ratio}` : ratio === "adaptive" ? "adaptive · 模型判断" : ratio}</option>)}</select></Field>
            <Field label="接口时长（4–15秒）" hint="MetaSo H3 按整数秒提交。舞台口播建议单镜 5–8 秒，动作复杂时优先拆镜。"><input type="number" min={4} max={15} step={1} value={Math.round(draft.duration_seconds)} onChange={(event) => patch({ duration_seconds: Math.max(4, Math.min(15, Math.round(Number(event.target.value) || 4))) })} /></Field>
            <Field label={effectiveMode === "r2v" ? "构图参考图（Picture 1）" : "首帧"} hint={effectiveMode === "r2v" ? "在全能参考中作为第一张构图参考，不会硬锁为视频第 0 秒。" : "可单独使用，也可与尾帧一起控制起点和终点。"}><select value={draft.keyframe_asset_id || ""} onChange={(event) => patch({ keyframe_asset_id: event.target.value || null })}><option value="">未绑定</option>{assets.filter((asset) => asset.type === "image").map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></Field>
            <Field label="尾帧" hint={effectiveMode === "r2v" ? "全能参考模式不会提交尾帧；切换到帧模式后生效。" : "可单独作为目标尾帧，也可与首帧组成首尾帧过渡。"}><select value={draft.last_frame_asset_id || ""} onChange={(event) => patch({ last_frame_asset_id: event.target.value || null })}><option value="">未绑定</option>{assets.filter((asset) => asset.type === "image").map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></Field>
            <Field label="Context IR" hint="用于增强复杂参考素材理解；开启后 MetaSo 会按次收取额外费用。"><select value={draft.h3_context_ir_enabled === null ? "default" : draft.h3_context_ir_enabled ? "on" : "off"} onChange={(event) => patch({ h3_context_ir_enabled: event.target.value === "default" ? null : event.target.value === "on" })}><option value="default">沿用模型设置</option><option value="off">关闭</option><option value="on">开启 · 额外计费</option></select></Field>
            <Field label="接口模型" hint="当前 MetaSo 通道固定提交 MiniMax-H3。"><input value="MiniMax-H3" disabled readOnly /></Field>
        </div>
        {effectiveMode === "r2v" && <div className="mt-4"><span className="studio-label">全能参考（可多选）</span><div className="mt-1 max-h-32 space-y-1 overflow-y-auto rounded-lg border border-white/8 p-2">{assets.filter((asset) => ["image", "video", "audio"].includes(asset.type)).map((asset) => <label key={asset.id} className="flex items-center gap-2 text-xs text-white/60"><input type="checkbox" checked={draft.reference_asset_ids.includes(asset.id)} onChange={() => patch({ reference_asset_ids: toggle(draft.reference_asset_ids, asset.id) })} />{asset.type.toUpperCase()} · {asset.name}</label>)}</div><p className="mt-1.5 text-[11px] leading-5 text-white/32">上限：图片 9 张、视频 3 个、音频 3 个，合计最多 12 项。构图参考图会计入图片数量；只绑定本镜真正需要的角色、动作和声音。</p></div>}
        <p className="mt-4 rounded-lg border border-cyan-300/10 bg-cyan-300/[.03] px-3 py-2.5 text-[11px] leading-5 text-cyan-100/55">本镜提交时会冻结：{inputLabel} · {resolutionLabel} · {ratioLabel} · Context IR {draft.h3_context_ir_enabled === null ? "沿用设置" : draft.h3_context_ir_enabled ? "开启" : "关闭"}。之后修改模型设置不会改变已经排队的任务。</p>
        ...
        {referencePreview && <MediaPreview preview={{ name: referencePreview.name, url: projectInlineUrl(projectId, "asset", referencePreview.id), description: referencePreview.description }} onClose={() => setReferencePreview(null)} />}
    </article>;
}

function SeedanceShotCard({ shot, project, assets, checked, busy, toggleChecked, save, projectId, keyframe, generatingKeyframe, editKeyframe, previewKeyframe, submit, canSubmit, previewReference }: { shot: Shot; project: Project; assets: Asset[]; checked: boolean; busy: boolean; toggleChecked: () => void; save: (shot: Shot) => Promise<void>; canSubmit: boolean } & ShotCardMediaProps) {
    const [draft, setDraft] = useState(shot);
    const [referencePreview, setReferencePreview] = useState<Asset | null>(null);
    ...
    const seedancePromptMaterials = numberedMaterials.map(({ id, name, type, label }) => ({ id, name, type, label }));
    const counts = {
    ...
    return <article className="rounded-xl border border-white/8 bg-black/15 p-4">
        {seedancePromptMaterials.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2 rounded-lg border border-cyan-300/10 bg-cyan-300/[.025] px-3 py-2"><span className="mr-1 text-[11px] text-white/40">参考缩略图</span>{seedancePromptMaterials.map((material) => { const asset = assets.find((item) => item.id === material.id); return <span key={material.id} className="inline-flex items-center gap-1.5 text-[11px] text-white/60"><PromptMaterialThumbnail material={material} getAssetUrl={(item) => projectInlineUrl(projectId, "asset", item.id)} onClick={asset?.type === "image" && previewReference ? () => previewReference(asset) : undefined} /><span>{material.label}</span></span>; })}</div>}
        <div className="flex flex-wrap items-start gap-3">
            <input type="checkbox" checked={checked} onChange={toggleChecked} />
...
function unlockedShotRenderStatus(shot: Shot, jobs: RenderJob[]): string {
    const videoJobs = jobs.filter((job) => job.type === "video" && job.shot_id === shot.id);
    const completedCount = videoJobs.filter((job) => job.status === "completed" && !!job.output_path).length;
    const activeJob = videoJobs.find((job) => ACTIVE_JOBS.has(job.status));
    if (activeJob) {
        const activeLabel = activeJob.status === "queued"
            ? "排队中"
            : activeJob.status === "cancel_requested"
                ? "取消中"
                : "生成中";
        return completedCount > 0 ? `${activeLabel} · 已有 ${completedCount} 个可选版本` : activeLabel;
    }
    if (completedCount > 0) return `已有 ${completedCount} 个可选版本 · 待锁定`;
    if (shot.video_path) return "已有自动版本 · 待锁定";
    if (videoJobs.some((job) => job.status === "failed")) return "最近生成失败";
    return "尚未生成";
}
...
function ProductionPanel({ bundle, busy, busyTasks, error, notice, action, refresh, notify, fail }: { bundle: ProjectBundle; busy: string; busyTasks: BusyState; error: string; notice: string; action: Action; refresh: () => Promise<void>; notify: (message: string) => void; fail: (message: string) => void }) {
    ...
    const [h3PromptInputMode, setH3PromptInputMode] = useState<VideoPromptInputMode>("");
    const [seedancePromptInputMode, setSeedancePromptInputMode] = useState<VideoPromptInputMode>("");
    ...
    const [renderCompletionOpen, setRenderCompletionOpen] = useState(false);
    ...
    const renderStats = summarizeRenderJobs(bundle.jobs, bundle.shots);
    const shotLockStats = summarizeShotLocks(bundle.jobs, bundle.shots);
    const totalRenderSeconds = renderStats.totalSeconds.toLocaleString("zh-CN", { maximumFractionDigits: 1 });
    const completedVideoJobs = bundle.jobs.filter((job) => job.type === "video" && job.status === "completed" && !!job.output_path);
    const selectedCompletedJobIds = selectedJobIds.filter((id) => completedVideoJobs.some((job) => job.id === id));
    const unlockedShots = bundle.shots.filter((shot) => shotLockStats.unlockedShotIds.includes(shot.id));
    const allLockedVideosSelected = shotLockStats.lockedVideoJobIds.length > 0 && shotLockStats.lockedVideoJobIds.every((id) => selectedCompletedJobIds.includes(id));
    const allUnadoptedVideosSelected = shotLockStats.unadoptedVideoJobIds.length > 0 && shotLockStats.unadoptedVideoJobIds.every((id) => selectedCompletedJobIds.includes(id));
    ...
    const promptInputMode = videoEngine === "h3" ? h3PromptInputMode : seedancePromptInputMode;
    ...
    const generateH3PromptSet = async (shotIds: string[], label: string) => {
        ...
        if (!h3PromptInputMode) {
            fail("请先选择这次要生成“帧模式 Prompt”还是“全能参考 Prompt”。");
            return;
        }
        ...
        try {
            await action(
                "h3-prompts",
                () => generateH3Prompts(bundle.project.id, shotIds, promptSkillId, h3PromptInputMode, promptSkillSuggestions.trim()),
                `已用“${selectedPromptSkill?.name || promptSkillId}”生成并保存${h3PromptInputMode === "frame" ? "帧模式" : "全能参考"}${label}。`,
            );
        } finally {
    ...
    const generateSelectedSeedancePrompts = () => {
        if (!seedancePromptInputMode) {
            fail("请先选择这次要生成“严格首帧 Prompt”还是“全能参考 Prompt”。");
            return Promise.resolve();
        }
        return action(
            "seedance-prompts",
            () => generateSeedancePrompts(bundle.project.id, selected, seedancePromptInputMode, promptSkillSuggestions.trim()),
            `已编译并保存 ${selected.length} 个 Seedance ${seedancePromptInputMode === "frame" ? "严格首帧" : "全能参考"} Prompt。`,
        );
    };
    ...
    const toggleCompletedVideoGroup = (groupIds: string[]) => setSelectedJobIds((current) => {
        const selectedIds = new Set(current.filter((id) => completedVideoJobs.some((job) => job.id === id)));
        const removeGroup = groupIds.length > 0 && groupIds.every((id) => selectedIds.has(id));
        groupIds.forEach((id) => removeGroup ? selectedIds.delete(id) : selectedIds.add(id));
        return Array.from(selectedIds);
    });
    const downloadSelectedVideos = () => action(
        "export-videos",
        async () => {
            const { blob, filename } = await exportRenderJobs(bundle.project.id, selectedCompletedJobIds);
            const url = URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.href = url;
            link.download = filename;
        ...
        `已将所选 ${selectedCompletedJobIds.length} 个视频打包下载。`,
        false,
    );
    ...
    const applyBulkH3Api = (values: Partial<Shot>, label: string) => action(
        `h3-api-${label}`,
        () => Promise.all(bundle.shots.filter((shot) => selected.includes(shot.id)).map((shot) => updateShot({ ...shot, ...values }))),
        `已将${label}应用到 ${selected.length} 个镜头`,
    );
    const submitSelected = () => action(
        "render",
        () => enqueueRender(bundle.project.id, selected, videoEngine === "seedance" ? { provider: "ark_seedance", modelId: seedanceModel, resolution: seedanceResolution, generateAudio: seedanceGenerateAudio } : { provider: "metaso_h3" }),
    ...
    const busyText = busy === "preflight" ? (videoEngine === "seedance" ? "正在检查方舟 API Key 与所选模型配置（不会创建任务）…" : "正在检查当前 H3 通道配置…")
        : busy === "keyframes" ? `正在调用分镜图模型生成 ${selected.length} 张首帧，请稍候…`
            ...
            : busy === "seedance-prompts" ? `正在为 ${selected.length} 个镜头编译 Seedance 2.0 ${seedancePromptInputMode === "frame" ? "严格首帧" : "全能参考"} Prompt…`
            : busy === "plan" ? "正在编译所有镜头的 H3 输入模式、参考素材与接口参数…"
                : busy === "render" ? `正在把 ${selected.length} 个${videoEngine === "seedance" ? " Seedance" : " H3"}镜头写入本地持久队列…`
                    : busy.startsWith("h3-api-") ? "正在保存所选镜头的 MetaSo 接口参数…"
    ...
    return <div className="space-y-5">
        <section className="studio-panel">
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><p className="studio-kicker">VIDEO ENGINE</p><h2 className="text-xl font-semibold">视频生成控制台</h2><p className="mt-1 text-sm text-white/40">H3 页面按 MetaSo MiniMax-H3 接口参数组织；进入页面时默认不勾选分镜，避免误提交。</p></div><div className="flex rounded-xl border border-white/10 bg-black/20 p-1"><button type="button" className={`rounded-lg px-4 py-2 text-sm ${videoEngine === "seedance" ? "bg-cyan-300 text-black" : "text-white/45 hover:text-white"}`} onClick={() => { setVideoEngine("seedance"); setPreflight(null); }}>Seedance 2.0 API</button><button type="button" className={`rounded-lg px-4 py-2 text-sm ${videoEngine === "h3" ? "bg-cyan-300 text-black" : "text-white/45 hover:text-white"}`} onClick={() => { setVideoEngine("h3"); setPreflight(null); }}>MetaSo H3 API</button></div></div>
            <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="studio-kicker">{videoEngine === "seedance" ? "VOLCENGINE ARK" : "METASO · MINIMAX-H3"}</p><h3 className="font-semibold">{videoEngine === "seedance" ? selectedSeedanceModel?.label || "Seedance 2.0" : "MetaSo H3 接口"}</h3><p className="mt-1 text-xs leading-5 text-white/38">先明确本次 Prompt 的输入类型；帧模板与全能参考模板、素材语法和后续接口提交保持一致。</p></div><div className="flex flex-wrap gap-2"><button className="studio-secondary" disabled={!!busy || (videoEngine === "seedance" && !seedanceCatalog)} title="只检查配置，不创建视频任务" onClick={() => void runPreflight()}>{busy === "preflight" ? <Loader2 className="animate-spin" size={15} /> : <Server size={15} />}检查配置</button><button className="studio-secondary" disabled={availableKeyframeIds.length === 0} title="调用分镜图服务，可与 H3 Prompt 并行；自动跳过正在处理首帧的镜头" onClick={() => void generateSelectedKeyframes()}>{selected.some((id) => pendingKeyframes.includes(id)) ? <Loader2 className="animate-spin" size={15} /> : <Sparkles size={15} />}生成所选首帧</button>{videoEngine === "seedance" ? <button className="studio-secondary" disabled={!!busy || selected.length === 0 || !seedancePromptInputMode} title={seedancePromptInputMode ? "只保存 Seedance 专用 Prompt，不调用付费视频接口" : "请先选择 Prompt 输入类型"} onClick={() => void generateSelectedSeedancePrompts()}>{busy === "seedance-prompts" ? <Loader2 className="animate-spin" size={15} /> : <WandSparkles size={15} />}编译所选 Prompt</button> : <button className="studio-secondary" disabled={!!busy || bundle.shots.length === 0} title="只整理输入模式、接口参数和参考绑定，不生成图片或视频" onClick={() => void action("plan", () => planRender(bundle.project.id), "编译完成：MetaSo H3 输入模式、接口参数和参考标签已保存。")}>{busy === "plan" ? <Loader2 className="animate-spin" size={15} /> : <Settings2 size={15} />}编译计划</button>}<button className="studio-primary" disabled={!!busy || selected.length === 0 || (videoEngine === "seedance" && (!seedanceCatalog?.configured || !seedanceEstimate))} title={videoEngine === "seedance" ? "提交后会产生实际方舟费用" : "提交后会产生 MetaSo H3 实际费用"} onClick={() => void submitSelected()}>{busy === "render" ? <Loader2 className="animate-spin" size={15} /> : <Play size={15} />}提交 {selected.length} 镜</button></div></div>
            <div className="mt-5 grid gap-2 text-xs sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border border-white/8 bg-white/[.02] p-3"><strong className="text-white/75">1. 检查配置</strong><p className="mt-1 leading-5 text-white/35">{videoEngine === "seedance" ? "验证 API Key、模型与清晰度；不会创建任务或计费。" : "验证 MetaSo API Key 与查询接口；不创建视频任务。"}</p></div>
                ...
                <div className="rounded-lg border border-white/8 bg-white/[.02] p-3"><strong className="text-white/75">3. 编译 Prompt</strong><p className="mt-1 leading-5 text-white/35">{videoEngine === "seedance" ? "生成方舟专用素材编号、镜头时序、说话人和声音指令。" : "整理帧模式/全能参考、Prompt、比例与清晰度。"}</p></div>
                <div className="rounded-lg border border-white/8 bg-white/[.02] p-3"><strong className="text-white/75">4. 提交镜头</strong><p className="mt-1 leading-5 text-white/35">{videoEngine === "seedance" ? "加入方舟异步队列；这是产生实际费用的一步。" : "冻结本镜接口参数并加入 MetaSo H3 持久队列。"}</p></div>
            </div>
            <div className="mt-4 rounded-xl border border-amber-300/18 bg-amber-300/[.035] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="studio-kicker text-amber-100/65">PROMPT INPUT · REQUIRED</p><h3 className="font-semibold">本次生成哪一种 Prompt？</h3><p className="mt-1 text-xs leading-5 text-white/40">这是生成前必选项，会写入所选镜头；之后提交视频沿用同一种输入通道，不再由台词或素材自动猜测。</p></div>{promptInputMode && <span className="studio-status text-amber-100">已选：{promptInputMode === "frame" ? (videoEngine === "h3" ? "帧模式" : "严格首帧") : "全能参考"}</span>}</div>
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                    <button type="button" aria-pressed={promptInputMode === "frame"} className={`rounded-xl border p-4 text-left transition ${promptInputMode === "frame" ? "border-cyan-300/50 bg-cyan-300/[.08]" : "border-white/10 bg-black/20 hover:border-white/25"}`} onClick={() => videoEngine === "h3" ? setH3PromptInputMode("frame") : setSeedancePromptInputMode("frame")}><strong className="block text-sm text-white/85">{videoEngine === "h3" ? "帧模式 Prompt（首帧 / 尾帧 / 首尾帧）" : "严格首帧 Prompt"}</strong><span className="mt-1 block text-xs leading-5 text-white/40">{videoEngine === "h3" ? "只使用 first_frame / last_frame API 字段，生成 I2V/首尾帧专用结构，不写全能参考素材编号。" : "只使用 @图片1 作为视频 0.00 秒硬首帧，不带人物图、场景图、视频或音频参考。"}</span></button>
                    <button type="button" aria-pressed={promptInputMode === "reference"} className={`rounded-xl border p-4 text-left transition ${promptInputMode === "reference" ? "border-cyan-300/50 bg-cyan-300/[.08]" : "border-white/10 bg-black/20 hover:border-white/25"}`} onClick={() => videoEngine === "h3" ? setH3PromptInputMode("reference") : setSeedancePromptInputMode("reference")}><strong className="block text-sm text-white/85">全能参考 Prompt</strong><span className="mt-1 block text-xs leading-5 text-white/40">使用图片、视频、音频等参考素材及对应编号；不把任何图片写成必须严格匹配的视频首帧或尾帧。</span></button>
                </div>
                <label className="mt-3 block"><span className="studio-label">本次 Prompt 生成建议（可选）</span><textarea className="studio-input mt-1 min-h-20 w-full resize-y" maxLength={4000} value={promptSkillSuggestions} onChange={(event) => setPromptSkillSuggestions(event.target.value)} placeholder="例如：动作更克制；保留当前群像构图；不要背景音乐；重点强化结尾悬念……" /><span className="mt-1 block text-[11px] leading-5 text-white/35">会作为本次 H3 / Seedance Prompt 生成的高优先级要求；不会修改已保存的模板。</span></label>
            </div>
            {videoEngine === "seedance" ? <div className="mt-4 rounded-xl border border-cyan-300/15 bg-cyan-300/[.025] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="studio-kicker">SEEDANCE MODEL & COST</p><h3 className="font-semibold">模型、清晰度、声音与实时费用</h3><p className="mt-1 max-w-3xl text-xs leading-5 text-white/40">按方舟公开 token 公式估算；时长向上取整且每镜至少 4 秒，最终金额以任务 usage 与账单为准。当前本地并发 {seedanceCatalog?.render_concurrency ?? "—"} 路，可在“模型与接口设置”调整（范围 1–8）。</p></div><div className="flex gap-3 text-xs"><a className="text-cyan-200/70 underline underline-offset-2" href={seedanceCatalog?.prompt_guide} target="_blank" rel="noreferrer">官方提示词指南</a><a className="text-cyan-200/70 underline underline-offset-2" href={seedanceCatalog?.pricing_source} target="_blank" rel="noreferrer">官方价格</a></div></div>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="studio-kicker">PROMPT SKILLS</p><h3 className="font-semibold">后期一键补生成 MiniMax H3 Prompt</h3><p className="mt-1 max-w-3xl text-xs leading-5 text-white/40">分镜阶段只选 Seedance 也没关系。这里会识别缺失、内容已更新或导演版本过期的镜头，一键补齐；只生成并保存 Prompt，可与首帧生成并行，不会启动视频生成。H3 与 Seedance 都会保留分镜明确写出的屏幕、标牌和界面内容；Seedance 默认不再追加通用禁字约束。</p></div><div className="flex items-center gap-2"><span className="studio-status text-emerald-200">已保存 {savedH3PromptCount}/{bundle.shots.length} · 待核对 {staleH3PromptCount}</span>{selectedPromptSkill?.source_url && <a className="text-xs text-cyan-200/70 underline underline-offset-2" href={selectedPromptSkill.source_url} target="_blank" rel="noreferrer">查看官方 Skill v{selectedPromptSkill.version}</a>}</div></div>
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/8 bg-black/15 p-3"><p className="text-xs leading-5 text-white/40">缺失 {absentH3PromptCount} 镜 · 待核对 {staleH3PromptCount} 镜。素材或剧情变化只标记待核对，已生成原文与历史仍会保留。点击补齐/更新才会再次调用文本模型。</p><div className="flex flex-wrap gap-2"><button type="button" className="studio-secondary" disabled={h3PromptBlocked || selected.length === 0 || promptSkills.length === 0 || !h3PromptInputMode} title={h3PromptInputMode ? "按上方所选输入类型重新生成" : "请先选择 Prompt 输入类型"} onClick={() => void generateSelectedH3Prompts()}>{h3PromptsBusy ? <Loader2 className="animate-spin" size={15} /> : <WandSparkles size={15} />}重新生成所选 {selected.length} 镜</button><button type="button" className="studio-primary" disabled={h3PromptBlocked || missingH3ShotIds.length === 0 || promptSkills.length === 0 || !h3PromptInputMode} title={h3PromptInputMode ? "按上方所选输入类型补齐" : "请先选择 Prompt 输入类型"} onClick={() => void generateMissingH3Prompts()}>{h3PromptsBusy ? <Loader2 className="animate-spin" size={15} /> : <Sparkles size={15} />}{missingH3ShotIds.length > 0 ? staleH3PromptCount > 0 ? `补齐/更新待核对 ${missingH3ShotIds.length} 镜` : `一键补齐待生成 ${missingH3ShotIds.length} 镜` : "H3 Prompt 已全部生成"}</button></div></div>
        ...
        {videoEngine === "seedance" ? <section className="studio-panel"><div className={`flex flex-wrap items-center justify-between gap-3 ${seedancePromptsOpen ? "mb-4" : ""}`}><div><p className="studio-kicker">SEEDANCE SHOTS</p><h3 className="font-semibold">逐镜 Seedance Prompt 与接口参数</h3><p className="mt-1 text-xs text-white/35">{seedancePromptsOpen ? "每个分镜内可核对首帧、Prompt、场景连续性并直接提交本镜。" : `当前已收起 ${bundle.shots.length} 个完整分镜，渲染队列显示在下方。`}</p></div><div className="flex flex-wrap gap-2">{seedancePromptsOpen && <><button className="studio-secondary" disabled={!selected.length || !!busy || !seedancePromptInputMode} title={seedancePromptInputMode ? "按上方所选输入类型重新编译" : "请先选择 Prompt 输入类型"} onClick={() => void generateSelectedSeedancePrompts()}>{busy === "seedance-prompts" ? <Loader2 className="animate-spin" size={15} /> : <WandSparkles size={15} />}重新编译所选 {selected.length} 镜</button><button className="studio-secondary" disabled={bundle.shots.length === 0} onClick={() => setSelected(selected.length === bundle.shots.length ? [] : bundle.shots.map((shot) => shot.id))}>{bundle.shots.length > 0 && selected.length === bundle.shots.length ? "取消全选" : "全选"}</button></>}<button type="button" className="studio-secondary px-3" aria-expanded={seedancePromptsOpen} aria-controls="seedance-prompts-content" onClick={() => setSeedancePromptsOpen((open) => !open)}>{seedancePromptsOpen ? <ChevronUp size={15} /> : <ChevronDown size={15} />}{seedancePromptsOpen ? "整体收起" : "整体展开"}</button></div></div>{seedancePromptsOpen && <div id="seedance-prompts-content">{bundle.shots.length === 0 ? <p className="rounded-lg border border-dashed border-white/10 p-5 text-sm text-white/35">请先在“分镜设计”中生成或导入分镜。</p> : <div className="space-y-3">{bundle.shots.map((shot) => { const keyframe = shot.keyframe_asset_id ? keyframeAssets.get(shot.keyframe_asset_id) : undefined; return <SeedanceShotCard key={`seedance-${shot.id}-${shot.updated_at}`} shot={shot} project={bundle.project} assets={bundle.assets} checked={selected.includes(shot.id)} busy={h3PromptsBusy || keyframeBusy(shot)} toggleChecked={() => setSelected(toggle(selected, shot.id))} save={saveSeedanceShot} projectId={bundle.project.id} keyframe={keyframe} generatingKeyframe={keyframeBusy(shot)} editKeyframe={() => openKeyframeEditor(shot)} previewKeyframe={() => keyframe && setKeyframePreview({ name: `镜头 ${shot.ordinal} 首帧`, url: projectDownloadUrl(bundle.project.id, "asset", keyframe.id), description: shot.keyframe_prompt || keyframe.description })} submit={(draft) => submitOneShot("seedance", draft)} canSubmit={!!seedanceCatalog?.configured} />; })}</div>}</div>}</section> : <section className="studio-panel"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">逐镜 MetaSo H3 Prompt 与接口参数</h3><p className="text-xs text-white/35">逐镜选择帧模式或全能参考，并保存比例、清晰度、时长与 Context IR。</p></div><div className="flex flex-wrap gap-2"><button className="studio-secondary" title="将所选镜头设为 MetaSo 768P" disabled={!selected.length || !!busy} onClick={() => void applyBulkH3Api({ h3_resolution: "768P" }, "768P 清晰度")}>批量 768P</button><button className="studio-secondary" title="将所选镜头设为 MetaSo 2K；会提高费用" disabled={!selected.length || !!busy} onClick={() => void applyBulkH3Api({ h3_resolution: "2K" }, "2K 清晰度")}>批量 2K</button><button className="studio-secondary" title="让所选镜头跟随项目画幅" disabled={!selected.length || !!busy} onClick={() => void applyBulkH3Api({ h3_ratio: "project" }, `项目比例 ${bundle.project.brief.aspect_ratio}`)}>批量项目比例</button><button className="studio-secondary" title="切换全部镜头的勾选状态，不修改参数" disabled={bundle.shots.length === 0} onClick={() => setSelected(selected.length === bundle.shots.length ? [] : bundle.shots.map((shot) => shot.id))}>{bundle.shots.length > 0 && selected.length === bundle.shots.length ? "取消全选" : "全选"}</button></div></div>
            <details open className="mb-4 rounded-xl border border-cyan-300/12 bg-cyan-300/[.025] p-4"><summary className="cursor-pointer text-sm font-semibold text-cyan-100/80">MetaSo H3 接口模式说明</summary><div className="mt-3 grid gap-2 text-xs sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-lg border border-white/8 bg-black/15 p-3"><strong className="text-white/75">首帧 / 尾帧 / 首尾帧</strong><p className="mt-1 leading-5 text-white/38">帧模式会硬约束起点或终点；同时绑定首尾帧可做连续过渡，画面比例由输入帧决定。</p></div><div className="rounded-lg border border-white/8 bg-black/15 p-3"><strong className="text-white/75">全能参考</strong><p className="mt-1 leading-5 text-white/38">图片、视频、音频可组合；与帧模式互斥，构图图会作为第一张参考图片提交。</p></div><div className="rounded-lg border border-white/8 bg-black/15 p-3"><strong className="text-white/75">768P / 2K</strong><p className="mt-1 leading-5 text-white/38">批量试跑优先 768P；确认 Prompt、人物和动作后，再把最终版本切到 2K。</p></div><div className="rounded-lg border border-white/8 bg-black/15 p-3"><strong className="text-white/75">Context IR</strong><p className="mt-1 leading-5 text-white/38">复杂参考素材理解不稳时再开启；MetaSo 会对它单独计费，默认沿用模型设置。</p></div></div></details>
            {bundle.shots.length === 0 ? <p className="rounded-lg border border-dashed border-white/10 p-5 text-sm text-white/35">请先在“分镜设计”中生成或导入分镜。</p> : <div className="space-y-3">{bundle.shots.map((shot, index) => { const keyframe = shot.keyframe_asset_id ? keyframeAssets.get(shot.keyframe_asset_id) : undefined; return <H3ShotCard key={`${shot.id}-${shot.updated_at}`} directorVersion={h3DirectorVersion} shot={shot} project={bundle.project} assets={bundle.assets} checked={selected.includes(shot.id)} busy={h3PromptsBusy || keyframeBusy(shot)} toggleChecked={() => setSelected(toggle(selected, shot.id))} save={saveShot} projectId={bundle.project.id} previousShotId={bundle.shots[index - 1]?.id || null} keyframe={keyframe} generatingKeyframe={keyframeBusy(shot)} editKeyframe={() => openKeyframeEditor(shot)} previewKeyframe={() => keyframe && setKeyframePreview({ name: `镜头 ${shot.ordinal} 首帧`, url: projectDownloadUrl(bundle.project.id, "asset", keyframe.id), description: shot.keyframe_prompt || keyframe.description })} submit={(draft) => submitOneShot("h3", draft)} />; })}</div>}</section>}
        <section className="studio-panel">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div><h3 className="font-semibold">渲染任务</h3><p className="text-xs text-white/35">视频任务 {renderStats.taskCount} 个 · 计划生成 {totalRenderSeconds} 秒 · 活动 {active.length} · 已记录估算成本 ¥{cost.toFixed(2)}</p></div>
                <div className="flex flex-wrap items-center gap-2">
                    <button type="button" className="studio-status inline-flex items-center gap-1.5" aria-expanded={renderCompletionOpen} aria-controls="render-completion-content" onClick={() => setRenderCompletionOpen((open) => !open)}>已完成分镜 {shotLockStats.lockedShotCount}/{shotLockStats.totalShotCount}{renderCompletionOpen ? <ChevronUp size={13} /> : <ChevronDown size={13} />}</button>
                    <button type="button" className="studio-secondary" disabled={shotLockStats.lockedVideoJobIds.length === 0} title="每个已锁定分镜只选择手动采用的版本" onClick={() => toggleCompletedVideoGroup(shotLockStats.lockedVideoJobIds)}>{allLockedVideosSelected ? "取消已锁定选择" : `全选已锁定分镜 (${shotLockStats.lockedVideoJobIds.length})`}</button>
                    <button type="button" className="studio-secondary" disabled={shotLockStats.unadoptedVideoJobIds.length === 0} title="选择全部未被手动采用的成功视频，包括未锁定分镜和已锁定分镜的其他备选版本" onClick={() => toggleCompletedVideoGroup(shotLockStats.unadoptedVideoJobIds)}>{allUnadoptedVideosSelected ? "取消备用素材选择" : `全选未锁定/未采用 (${shotLockStats.unadoptedVideoJobIds.length})`}</button>
                    {completedVideoJobs.length > 0 && <button type="button" className="studio-primary" disabled={selectedCompletedJobIds.length === 0 || busy === "export-videos"} onClick={() => void downloadSelectedVideos()}>{busy === "export-videos" ? <Loader2 className="animate-spin" size={14} /> : <Download size={14} />}下载已选 {selectedCompletedJobIds.length > 0 ? `(${selectedCompletedJobIds.length})` : ""}</button>}
                    <button className="studio-secondary" onClick={() => void refresh()}><RefreshCw size={14} />刷新</button>
                </div>
            </div>
            {renderCompletionOpen && <div id="render-completion-content" className="mb-4 rounded-xl border border-cyan-300/12 bg-cyan-300/[.025] p-3">
                {unlockedShots.length === 0
                    ? <p className="text-sm text-emerald-200/75"><Check size={14} className="mr-1.5 inline" />全部 {bundle.shots.length} 个分镜都已手动锁定采用版本。</p>
                    : <><p className="mb-2 text-xs text-white/45">以下 {unlockedShots.length} 个分镜还未锁定采用版本：</p><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{unlockedShots.map((shot) => <div key={`unlocked-${shot.id}`} className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-white/8 bg-black/15 px-3 py-2"><span className="min-w-0 truncate text-xs text-white/70">镜头 {shot.ordinal}{shot.title ? ` · ${shot.title.replace(/^镜头\s*\d+\s*[·.:：-]?\s*/, "")}` : ""}</span><span className="shrink-0 text-[11px] text-amber-100/60">{unlockedShotRenderStatus(shot, bundle.jobs)}</span></div>)}</div></>}
            </div>}
            {bundle.jobs.length === 0 ? <p className="text-sm text-white/35">尚未提交任务。</p> : <div className="space-y-2">{bundle.jobs.map((job) => {
            ...
            return <JobRow key={job.id} job={job} projectId={bundle.project.id} shotLabel={shot ? `镜头 ${shot.ordinal}` : "未绑定镜头"} selected={selected} currentOutput={currentOutput} downloadSelected={selectedCompletedJobIds.includes(job.id)} toggleDownload={() => setSelectedJobIds((current) => toggle(current, job.id))} preview={() => setVideoPreview({ name, url: projectInlineUrl(bundle.project.id, "job", job.id), downloadUrl: projectDownloadUrl(bundle.project.id, "job", job.id), provider })} adopt={shot ? () => action(`select-${job.id}`, () => selectRenderJob(bundle.project.id, job.id), `镜头 ${shot.ordinal} 已采用此版本；对应尾帧已锁定。`) : undefined} cancel={() => action(`cancel-${job.id}`, () => cancelRenderJob(bundle.project.id, job.id), "已发送取消请求")} remove={() => action(`delete-${job.id}`, () => deleteRenderJob(bundle.project.id, job.id), "已删除该渲染任务记录")} />;
        })}</div>}</section>
...
function DeliveryPanel({ bundle, busy, action }: { bundle: ProjectBundle; busy: string; action: Action }) {
    ...
    return <div className="space-y-5">
        <section className="studio-panel">
            <div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><p className="studio-kicker">PROJECT COVER</p><h2 className="text-xl font-semibold">项目封面图</h2><p className="mt-1 text-sm leading-6 text-white/40">自动结合项目标题、剧情、人设图、画风图和分镜首帧，生成类似剧情科普海报的主封面。生成前可写本次建议并单独选择比例。</p></div><span className="studio-status">{coverAssets.length} 个版本</span></div>
            <div className="grid gap-5 lg:grid-cols-[minmax(300px,.9fr)_minmax(360px,1.1fr)]">
                <div>
                    <div className="flex max-h-[540px] items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black/35" style={{ aspectRatio: currentCoverRatio.replace(":", " / ") }}>
                        {currentCover ? <button type="button" className="group relative h-full w-full cursor-zoom-in" onClick={() => setCoverPreview({ name: currentCover.name, url: projectInlineUrl(bundle.project.id, "asset", currentCover.id), description: currentCover.description })}><img className="h-full w-full object-contain transition-transform duration-200 group-hover:scale-[1.01]" src={projectInlineUrl(bundle.project.id, "asset", currentCover.id)} alt={currentCover.name} /><span className="absolute bottom-3 right-3 rounded-lg bg-black/70 px-3 py-2 text-xs text-white/75 opacity-0 backdrop-blur transition-opacity group-hover:opacity-100"><Maximize2 className="mr-1 inline" size={14} />查看原图</span></button> : <div className="flex flex-col items-center gap-3 px-8 text-center text-white/28"><ImageIcon size={34} className="text-cyan-300/55" /><strong className="text-sm text-white/45">还没有项目封面</strong><span className="text-xs leading-5">右侧填写建议并生成；已有的人设、画风和分镜图会自动作为参考。</span></div>}
                    </div>
                    {currentCover && <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><p className="text-xs text-white/35">当前版本 · {currentCoverRatio} · {(currentCover.size_bytes / 1024 / 1024).toFixed(1)}MB</p><div className="flex gap-2"><button type="button" className="studio-secondary px-3 py-2 text-xs" onClick={() => setCoverPreview({ name: currentCover.name, url: projectInlineUrl(bundle.project.id, "asset", currentCover.id), description: currentCover.description })}><Maximize2 size={14} />预览</button><a className="studio-primary px-3 py-2 text-xs" href={projectDownloadUrl(bundle.project.id, "asset", currentCover.id)}><Download size={14} />下载封面</a></div></div>}
                </div>
...
function JobRow({ job, projectId, shotLabel, selected, currentOutput, downloadSelected, toggleDownload, preview, adopt, cancel, remove }: { job: RenderJob; projectId: string; shotLabel: string; selected: boolean; currentOutput: boolean; downloadSelected: boolean; toggleDownload: () => void; preview: () => void; adopt?: () => Promise<void>; cancel: () => Promise<void>; remove: () => Promise<void> }) {
    ...
    const seedance = (job.input_snapshot?.seedance || {}) as Record<string, unknown>;
    const metaso = (job.input_snapshot?.metaso || {}) as Record<string, unknown>;
    const mediaQc = (job.input_snapshot?.media_qc || {}) as Record<string, unknown>;
    const mediaWarnings = Array.isArray(mediaQc.warnings) ? mediaQc.warnings.map(String) : [];
    const metasoInputMode = String(metaso.input_mode || params.api_input_mode || "历史任务输入");
    const metasoModeLabel = ({ references: "全能参考", first_last_frame: "首尾帧", first_frame: "首帧", last_frame: "尾帧", frame: "帧模式" } as Record<string, string>)[metasoInputMode] || metasoInputMode;
    const parameterText = job.provider === "ark_seedance"
        ...
        : job.provider === "metaso_h3"
            ? `${metasoModeLabel} · ${metaso.resolution || params.api_resolution || "默认清晰度"} · ${metaso.ratio || params.api_ratio || "默认比例"} · Context IR ${(metaso.context_ir_enabled ?? params.api_context_ir_enabled) ? "开启" : "关闭"}`
