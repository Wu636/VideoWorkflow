...
function ProductionPanel({ bundle, busy, busyTasks, error, notice, action, refresh, notify, fail }: { bundle: ProjectBundle; busy: string; busyTasks: BusyState; error: string; notice: string; action: Action; refresh: () => Promise<void>; notify: (message: string) => void; fail: (message: string) => void }) {
    ...
    const allCompletedVideosSelected = completedVideoJobs.length > 0 && selectedCompletedJobIds.length === completedVideoJobs.length;
    ...
    const toggleAllCompletedVideos = () => setSelectedJobIds(allCompletedVideosSelected ? [] : completedVideoJobs.map((job) => job.id));
    ...
    return <div className="space-y-5">
        ...
        <section className="studio-panel"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">渲染任务</h3><p className="text-xs text-white/35">视频任务 {renderStats.taskCount} 个 · 计划生成 {totalRenderSeconds} 秒 · 活动 {active.length} · 已记录估算成本 ¥{cost.toFixed(2)}</p></div><div className="flex flex-wrap items-center gap-2">{completedVideoJobs.length > 0 && <><span className="studio-status">已完成视频 {selectedCompletedJobIds.length}/{completedVideoJobs.length}</span><button type="button" className="studio-secondary" onClick={toggleAllCompletedVideos}>{allCompletedVideosSelected ? "取消全选" : "全选"}</button><button type="button" className="studio-primary" disabled={selectedCompletedJobIds.length === 0 || busy === "export-videos"} onClick={() => void downloadSelectedVideos()}>{busy === "export-videos" ? <Loader2 className="animate-spin" size={14} /> : <Download size={14} />}下载已选 {selectedCompletedJobIds.length > 0 ? `(${selectedCompletedJobIds.length})` : ""}</button></>}<button className="studio-secondary" onClick={() => void refresh()}><RefreshCw size={14} />刷新</button></div></div>{bundle.jobs.length === 0 ? <p className="text-sm text-white/35">尚未提交任务。</p> : <div className="space-y-2">{bundle.jobs.map((job) => {
