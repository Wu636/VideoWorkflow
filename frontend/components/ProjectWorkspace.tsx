...
function H3ShotCard({ shot, project, assets, checked, busy, directorVersion, toggleChecked, save, projectId, keyframe, generatingKeyframe, editKeyframe, previewKeyframe, submit, previousShotId }: { shot: Shot; project: Project; assets: Asset[]; checked: boolean; busy: boolean; directorVersion: string; toggleChecked: () => void; save: (shot: Shot) => Promise<void>; previousShotId: string | null } & ShotCardMediaProps) {
...
function ProductionPanel({ bundle, busy, busyTasks, error, notice, action, refresh, notify, fail }: { bundle: ProjectBundle; busy: string; busyTasks: BusyState; error: string; notice: string; action: Action; refresh: () => Promise<void>; notify: (message: string) => void; fail: (message: string) => void }) {
    ...
    const generateSelectedSeedancePrompts = () => {
        ...
        return action(
            "seedance-prompts",
            () => generateSeedancePrompts(bundle.project.id, selected, seedancePromptInputMode),
